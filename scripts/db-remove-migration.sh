...
echo "Attempting to remove the last migration..."
if ! dotnet ef migrations remove --project "${PERSISTENCE_PROJECT}" --startup-project "${STARTUP_PROJECT}"; then
    log_error "Failed to remove migration. Check if any migrations exist or if the database is in sync."
fi
