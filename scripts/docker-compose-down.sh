...
echo "Stopping Docker Compose environment defined in: ${DOCKER_COMPOSE_FILE}"
# Add -v to remove volumes if desired (destructive)
if ! ${COMPOSE_CMD} -f "${DOCKER_COMPOSE_FILE}" down; then
    log_error "Docker Compose down command failed."
fi
